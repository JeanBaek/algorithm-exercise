const fs = require("fs");
const filepath = "./30014.input.txt"; // process.platform === 'linux' ? '/dev/stdin' : 'input.txt';
...
function solution([N, P]: string[]) {
    ...
    // const X = sortedPList.reduce((prev, curr, idx, arr) => {
    //     console.log({prev, curr, idx, arr})
    //     if (idx === sortedPList.length - 1) {
    //         // it is the last
    //         return prev += curr * arr[0]
    //     }
    //
    //     return prev += curr * arr[idx + 1]
    // }, 0)
...
// TODO: N만큼 배열 크기 만들어놓고 양쪽에서 타고 가운데로 들어오기
